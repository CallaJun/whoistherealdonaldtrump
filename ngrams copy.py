...
class Ngrams:
    ...
    def generateTweets(self, n):
        ...
        word = self.weightedPickN(words, ngrams)
        while (len(tweet) + len(word) < 140) or (word != self.end_token): 
            if n != 1:
                del words[0]
                words.append(word)
            tweet += word
...
def parse_args():
    ...
    parser.add_argument("-tweet", "--tweet", action="store_true",
                        help="Generate tweet based on unsmoothed "
                             "n-grams of the training set.")
    ...
    parser.usage = ("ngrams.py [-h] [-n N] -sent -tweet training_set\n              "
                    "   [-h] [-n N] [-sent] [-tweet] [-t T] [-ls [\u03B1] | -gts] [-p] "
